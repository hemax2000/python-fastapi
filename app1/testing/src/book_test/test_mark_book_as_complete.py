...
def test_mark_book_as_complete(client: TestClient, seed_data: SeedData, create_a_book_for_1_user):
    ...
    assert res.json()[0]["completed"] == False

    book_id = res.json()[0]["id"]

    res = client.post(
        url=f"/book/{book_id}/complete",
        headers={"Authorization": f"Bearer {seed_data.users_tokens[0]}"},
    )
    assert res.status_code == 200
    ...
    assert res.status_code == 200
    assert len(res.json()) == 1
    assert res.json()[0]["completed"] == True
